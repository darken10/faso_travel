<div>
    ...
    <div class="flex items-center justify-between mb-6">
        <div>
            <h2 class="text-xl font-bold text-gray-800">Recettes manuelles</h2>
            <p class="text-sm text-gray-500 mt-0.5">{{ $recettes->total() }} enregistrements</p>
        </div>
    ...
    <div class="mb-4 bg-green-50 border border-green-100 rounded-xl px-4 py-3 flex items-center justify-between">
        <span class="text-sm text-green-700 font-medium">Total recettes (filtre actif)</span>
        <span class="text-xl font-bold text-green-600">{{ number_format($totalFiltre, 0, ',', ' ') }} F</span>
    </div>
    ...
    <div class="bg-white rounded-xl border border-gray-200 overflow-hidden shadow-sm">
            ...
            <tbody class="divide-y divide-gray-100">
                @forelse($recettes as $rec)
                    <tr class="hover:bg-gray-50 transition-colors">
                        <td class="px-4 py-3 font-medium text-gray-800">{{ $rec->libelle }}</td>
                        <td class="px-4 py-3 text-gray-500">{{ $rec->source ?? '—' }}</td>
                        ...
                        <td class="px-4 py-3 text-right">
                            <div class="flex items-center justify-end gap-1">
                                ...
                                <button wire:click="delete({{ $rec->id }})" wire:confirm="Supprimer cette recette ?" class="p-1.5 text-gray-400 hover:text-red-600 hover:bg-red-50 rounded-lg transition-colors">
        ...
        @if($recettes->hasPages())
            <div class="px-4 py-3 border-t border-gray-100">{{ $recettes->links() }}</div>
        @endif
    </div>
        ...
        <div class="fixed inset-0 z-50 flex items-start justify-center p-4 overflow-y-auto" x-data x-on:keydown.escape.window="$wire.set('showModal', false)">
            ...
            <div class="relative bg-white rounded-2xl shadow-2xl w-full max-w-lg my-4" x-trap.noscroll="true">
                <div class="flex items-center justify-between px-6 py-4 border-b border-gray-100">
                    <h3 class="text-base font-semibold text-gray-800">{{ $editingId ? 'Modifier la recette' : 'Nouvelle recette' }}</h3>
                ...
                <form wire:submit="save" class="px-6 py-5 space-y-4">
                    <div>
                        ...
                        <input wire:model="libelle" type="text" class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-400" placeholder="Ex: Location local">
                    ...
                    <div class="grid grid-cols-2 gap-3">
                        <div>
                            ...
                            <input wire:model="montant" type="number" min="1" class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-400" placeholder="Ex: 200000">
                            @error('montant') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">Date *</label>
                            <input wire:model="date_recette" type="date" class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-400">
                            @error('date_recette') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                        </div>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Source</label>
                        <input wire:model="source" type="text" class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-400" placeholder="Ex: Partenariat, subvention...">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Référence</label>
                        <input wire:model="reference" type="text" class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-400" placeholder="Ex: REC-2024-001">
                    </div>
                    <div>
