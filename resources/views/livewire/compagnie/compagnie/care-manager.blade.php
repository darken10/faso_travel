    ...
    <div class="bg-white rounded-xl border border-gray-200 overflow-hidden shadow-sm">
        <div class="px-4 py-3 border-b border-gray-100">
            <div class="relative">
                ...
                <input wire:model.live.debounce.300ms="search" type="text" placeholder="Rechercher par immatriculation..." class="w-full pl-9 pr-4 py-2 text-sm border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-400">
            </div>
        </div>
        <table class="w-full text-sm">
            <thead class="bg-gray-50 border-b border-gray-200">
                <tr>
                    ...
                    <th class="text-left px-4 py-3 font-semibold text-gray-600">Immatriculation</th>
            ...
            <tbody class="divide-y divide-gray-100">
                @forelse($cares as $care)
                    <tr class="hover:bg-gray-50 transition-colors">
                        <td class="px-4 py-3">
                            @if($care->image_uri)
                                <img src="{{ Storage::url($care->image_uri) }}" alt="{{ $care->immatrculation }}" class="w-12 h-9 rounded object-cover border border-gray-200">
                            @else
                                <div class="w-12 h-9 rounded bg-gray-100 flex items-center justify-center">
                                    <svg class="w-5 h-5 text-gray-300" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 16H6a2 2 0 01-2-2V6a2 2 0 012-2h8a2 2 0 012 2v2m-6 12h8a2 2 0 002-2v-8a2 2 0 00-2-2h-8a2 2 0 00-2 2v8a2 2 0 002 2z"/></svg>
                                </div>
                            @endif
                        </td>
                        <td class="px-4 py-3 font-mono font-semibold text-gray-800">{{ $care->immatrculation }}</td>
                        <td class="px-4 py-3 text-gray-600">{{ $care->number_place }}</td>
                        <td class="px-4 py-3">
                            @php $color = match($care->statut->value ?? '') { 'Disponible' => 'green', 'Occuper' => 'blue', 'En Panne' => 'red', default => 'gray' }; @endphp
                            <x-panel.badge :color="$color">{{ $care->statut?->value }}</x-panel.badge>
                        </td>
                        <td class="px-4 py-3 text-gray-500">{{ $care->etat ?? '—' }}</td>
                        <td class="px-4 py-3 text-right">
                            <div class="flex items-center justify-end gap-1">
                                <button wire:click="openEdit({{ $care->id }})" class="p-1.5 text-gray-400 hover:text-blue-600 hover:bg-blue-50 rounded-lg transition-colors">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>
                                </button>
                                <button wire:click="delete({{ $care->id }})" wire:confirm="Supprimer ce véhicule ?" class="p-1.5 text-gray-400 hover:text-red-600 hover:bg-red-50 rounded-lg transition-colors">
                ...
                @empty
                    <tr><td colspan="6" class="px-4 py-10 text-center text-gray-400">Aucun véhicule trouvé</td></tr>
                @endforelse
            </tbody>
        </table>
        @if($cares->hasPages())
            <div class="px-4 py-3 border-t border-gray-100">{{ $cares->links() }}</div>
        @endif
    </div>
        ...
        <div class="fixed inset-0 z-50 flex items-center justify-center p-4" x-data x-on:keydown.escape.window="$wire.set('showModal', false)">
            ...
            <div class="relative bg-white rounded-2xl shadow-2xl w-full max-w-md" x-trap.noscroll="true">
                <div class="flex items-center justify-between px-6 py-4 border-b border-gray-100">
                    <h3 class="text-base font-semibold text-gray-800">{{ $editingId ? 'Modifier le véhicule' : 'Nouveau véhicule' }}</h3>
                ...
                <form wire:submit="save" class="px-6 py-5 space-y-4" enctype="multipart/form-data">
                    ...
                    <div>
                        ...
                        <input wire:model="image" type="file" accept="image/*" class="w-full text-sm text-gray-500 file:mr-3 file:py-1.5 file:px-3 file:rounded-lg file:border-0 file:text-xs file:font-semibold file:bg-blue-50 file:text-blue-700 hover:file:bg-blue-100">
